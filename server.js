  ...
  ws.on('message', raw => {
    ...
    switch (msg.type) {
      ...
      case 'combo_heal': {
        ...
        if (!myKey) break;
        const defKey = myKey === 'p1' ? 'p2' : 'p1';
        sendTo(sess[defKey].ws, { type: 'combo_heal', sessionId, total });
        break;
      }

      case 'extra_crit': {
        const { sessionId, dmg } = msg;
        const sess = duelSessions.get(sessionId);
        if (!sess) break;
        const myKey  = sess.p1.id === id ? 'p1' : sess.p2.id === id ? 'p2' : null;
        if (!myKey) break;
        const defKey = myKey === 'p1' ? 'p2' : 'p1';
        sendTo(sess[defKey].ws, { type: 'extra_crit', sessionId, dmg });
        break;
      }
      ...
      case 'combo_anim': {
        ...
        if (!myKey) break;
        const defKey = myKey === 'p1' ? 'p2' : 'p1';
        sendTo(sess[defKey].ws, { type: 'combo_anim', sessionId });
        break;
      }
